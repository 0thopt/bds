function [xopt, fopt, exitflag, output] = bds_development(fun, x0, options)
    ...
    for iter = 1:maxit
        ...
        % Actually, the way of recording direction_index_hist is not reasonable, since the directions in one
        % block may not be visited fully. However, when we use estimated gradient to stop the algorithm,
        % we only consider the case where all the directions in one block are visited all. We will try to
