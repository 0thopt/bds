function [xopt, fopt, exitflag, output] = fminunc_with_history(fun, x0, options)
% fminunc_with_history(fun, x0, options)
% Returns:
...
% column vectors in output.xhist.
    ...
    % Call fminunc normally, but use the logger as the objective function
    [xopt, fopt, exitflag, output] = fminunc(@logged_fun, x0, options);
