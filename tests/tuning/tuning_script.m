...
% parameters.func_window_size = [3 4];
% parameters.grad_window_size = [3 5 6 8 10 12 15];
% parameters.func_tol = 10.^([-15 -12 -10 -8 -6 -5 -4 -3]);
% parameters.func_tol = 10.^([-15 -12]);
% parameters.func_tol_ratio = 1e-3;
% parameters.grad_tol = 10.^([-15 -12 -10 -8 -6 -5 -3]);
...
parameters.expand = 1.2:0.1:1.3;
parameters.shrink = 0.5:0.1:0.6;
options.mindim = 1;
options.maxdim = 1;
...
options.tau_weights(1, 1:options.max_tol_order, 1, 1) = [0.96/(options.max_tol_order-2)*ones(1, options.max_tol_order-2), 0.02, 0.02]; 
